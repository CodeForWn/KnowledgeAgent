...
class PromptBuilder:
    ...
    def generate_question_agent_prompt_for_qwen(knowledge_point, related_texts, spo, difficulty_level, question_type, question_count):
        ...
        example_dict = {
            ("单选题", "简单"): """试题示例格式参考：
        1. 板块构造学说可以帮助我们认识地球环境、解释地球运动的机理。下列地区，因板块碰撞而形成的是（　　）
        ...
        """,
            ("单选题", "困难"): """试题示例格式参考：
        1. 针对复杂地质构造和板块运动理论，请选择最能描述板块边界处地形形成的地区（　　）
        ...
        """,
            ("多选题", "困难"): """试题示例格式参考：
        1. 关于地球构造及其演变，哪些观点能较全面地解释板块运动与地貌形成之间的关系？
        ...
        """,
            ("填空题", "简单"): """试题示例格式参考：
        1. 岩石中包含的___可以反映古地理环境和生物特征，生物总是从___到___，从___到___ 。""",
            ("填空题", "普通"): """试题示例格式参考：
        1. 地球的自转周期为T1，近地卫星公转周期为T2，则他们的大小关系满足T1___T2（选填“＞”“＜”“=”）；已知万有引力常故为G，则地球的密度约为___（用前述字母表示）。""",
            ("填空题", "困难"): """试题示例格式参考：
        1. 地理信息技术是指获取、管理、分析和应用地理空间信息的现代技术的总称，主要包括（___英文缩写为___）、（___英文缩写为___）、（___英文缩写为___ ）等．"""
        }
        ...
        if related_texts:
            ...
            if spo and 'entity_relations' in spo:
                ...
                texts_context = "请结合你的高中地理知识自由设计题目。"
    ...
    def generate_ppt_from_outline_prompt(knowledge_point, outline_markdown, textbook_text):
        system_content = (
            "## Role：教学课件内容生成专家\n\n"
            "## 任务描述\n"
            "你是一位具备教学设计经验的AI课件内容生成专家，能够根据提供的教材全文内容，智能补充教学课件内容。\n\n"
            "## 任务目标\n"
            "- 教材内容理解：充分理解提供的教材全文，找出与主题知识点相关的关键信息。\n"
            "- 大纲内容扩展：根据提供的Markdown格式大纲，在每个标题下撰写对应的讲解内容。\n"
            "- 内容层次清晰：不同级别标题（#、##、###）下的内容应符合层次要求，主标题给出概览，子标题具体展开。\n"
            "- 表达规范统一：正文内容采用列表（- 正文内容）形式罗列，语言条理清晰，适合教学使用。\n"
            "- 教学辅助信息：适当增加图示、例题、经典文化引用等丰富内容（如适用）。\n\n"
            "## 输出要求\n"
            "- 保留原有Markdown的大纲结构（标题层级不能变化）\n"
            "- 在每个标题下补充若干条正文内容，每条前加'- '，形成清晰条目\n"
            "- 正文中不要添加额外的新的标题\n"
            "- 不要出现 ** 加粗符号、--- 分割线等无关Markdown格式\n"
            "- 最终输出为完整、规范的Markdown文档\n"
        )
