...
- 推理大于5步，需要跨章节知识综合运用，符合考研或竞赛高水平题型标准。"""
    ...
    D．L(G[N])={b2i+1│i ≥ 1}""",
            ("单选题", "困难"): """示例：
    已知语言L={anbbn|n≥1}，则下述文法中，（）可以产生语言L。
    A. Z → aZb | aAb | b
    A → aAb | b         
    B. A → aAb
    A → b
...
class PromptBuilder:
    ...
    def summarize_graph_relations(triplet_sentences, subject_name=None):
        ...
        """
        system_msg = "你是一位善于梳理知识图谱关系结构的专家。请根据下列多个知识点关系表达，将这些零散的关系梳理、归纳，合成为一段简洁且逻辑清晰的整体描述，突出核心概念、主要结构与逻辑链条。仅输出描述即可。"
        if subject_name:
    ...
    请合并整理为一段整体描述，不要逐条复述，而是像老师为学生整体讲解本主题知识结构一样表达，要努力挖掘出这些关系带来的隐含的，复杂的关系。语言要简明、准确、有条理。这段描述的目的是为了后续针对这个知识点出题的时候，能够生成一道符合改知识点在教学考纲中的要求，并且不会超纲的题目。
    """
        messages = [
